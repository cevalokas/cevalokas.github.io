...
[台大机器学习笔记](https://beader.me/mlnotebook/index.html)
[台大机器学习代码](https://github.com/AaronYALai/Machine_Learning_Techniques.git)

![](image/test/7b4d219e0bc34951e05cc1031c8fe87.png)

重点考试内容
- Fundamental concepts
- Parameter estimation: LSE Vs MLE
- Feed-forward network structure, Bayesian version and connection to GP
- Relation between Bayesian linear regression and GP
- GP regression, two major tasks, questions given on the white board
- Kernel trick, SE kernel = Infinite mapping
- Relation between Bayesian logistic regression and GPC
- Sparse model fundamentals, L1 norm regularization vs L2 norm regularization_
不考内容
- CRLB
- Practical GPR and GPC implementations
- Decision theory for GPR
- Backpropagation algorithm
- Bayesian model selection (derivations)

#### arg:    变元（即自变量argument）的英文缩写。
...
RULE激活函数
![](...\images\test\Pastedimage20231114204258.png)

w.r.t.   with respect to 常用于求导，或者满足一定条件之类的情况
 
 MSE:    均方误差
![](test\Pastedimage20231117103334.png)

 A ≜ B  定义A为B

凸偏好：
![](test\Pastedimage20231106225404.png)

协方差cov(x)>0正相关<0负相关，协方差矩阵由方差和协方差组成，是一个对称矩阵
m个样本，$\Sigma=\frac{1}{m}XX_T$  **总体是除以m，样本是m-1，因为要保证无偏估计。在矩阵中用数学期望算更方便
![](test\Pastedimage20231106221247.png)

**S(β)  $\Rightarrow$ β的残差平方和
残差在数理统计中是指实际观察值与估计值（拟合值）之间的差，一般用 “ e ” 表示** 
![](test\Pastedimage20231106234012.png)
![](test\Pastedimage20231106231921.png)
...
**黑塞矩阵**Hessian matrix：由多变量[实值函数](https://zh.wikipedia.org/wiki/%E5%AE%9E%E5%87%BD%E6%95%B0 "实函数")的所有二阶[偏导数](https://zh.wikipedia.org/wiki/%E5%81%8F%E5%AF%BC%E6%95%B0 "偏导数")组成的[方块矩阵](https://zh.wikipedia.org/wiki/%E6%96%B9%E5%9D%97%E7%9F%A9%E9%98%B5 "方块矩阵")![](test\Pastedimage20231108164655.png)
...
![](test\Pastedimage20231114204311.png)
![](test\Pastedimage20231106102820.png)
![](test\Pastedimage20231106103223.png)
...
![](test\Pastedimage20231106115228.png)
...
**一般情况下费雪信息是对数似然函数关于参数θ的二阶导数的负期望值**
 ![](test\Pastedimage20231107000011.png)
![](test\Pastedimage20231106235723.png)![](test\Pastedimage20231106235829.png)
...
![](test\Pastedimage20231107000030.png)
...
逻辑回归：
![](test\Pastedimage20231108123205.png)
![](test\Pastedimage20231108162648.png)
...
- ![](test\Pastedimage20231114223031.png)
- 迭代算法： $W^ {\tau +1} = W^{\tau} -\eta \nabla E(W^ {\tau})$
- [交叉熵损失函数](https://www.zhihu.com/tardis/zm/art/35709485?source_id=1003)![](test\Pastedimage20231114223215.png)
...
- ![](test\Pastedimage20231114234105.png)
- ![](test\Pastedimage20231114233936.png)
- 
...
- ![](test\Pastedimage20231115160006.png)
- 后验吸收了似然性和先验的知识，并捕获了我们所知道的有关模型参数的所有信息。
- 权重的高斯先验:
- ![](test\Pastedimage20231109232001.png)
...
- 高维高斯分布：![](test\Pastedimage20231109150609.png)
- 机器学习的训练过程目标就是学习该无限维高斯分布的子集——也是一个多元高斯分布的参数：均值向量和协方差矩阵
- ![](test\Pastedimage20231115174446.png)
- ![](test\Pastedimage20231109150937.png)
- 


牛顿法迭代公式：
![](test\Pastedimage20231116234901.png)


L1正则先验分布是Laplace分布，L2正则先验分布是Gaussian分布
laplace概率分布函数：
![](test\Pastedimage20231110154643.png)
laplace先验概率：
![](test\Pastedimage20231110154834.png)

We introduce indicator variables $\gamma_j$ , j = 1, 2, ..., p, whose value  
indicates whether the j-th feature is “in” or “out” of our model
![](test\Pastedimage20231110164125.png)
...
- 条件作用也是封闭的
- ![](test\Pastedimage20231116164629.png)
- 在高斯过程中，我们把每个测试点作为一个随机变量，多元高斯分布的维数和随机变量的数目一致。
- ![](test\Pastedimage20231116223222.png)
- ![](test\Pastedimage20231116223019.png)
- ![](test\Pastedimage20231116223036.png)
- ![](test\Pastedimage20231116223343.png)
- ![](test\Pastedimage20231116225713.png)
- 方程视角：![](test\Pastedimage20231117001432.png)Z是测试输入
- ![](test\Pastedimage20231116231435.png)
- ![](test\Pastedimage20231117001532.png)
- 机器学习的训练过程目标就是学习该无限维高斯分布的子集——也是一个多元高斯分布的参数：均值向量和协方差矩阵
- ![](test\Pastedimage20231117002410.png)
- ![](test\Pastedimage20231109150937.png)
...
- 最常用的SE kernel：
- ![](test\Pastedimage20231116173243.png)
...
![](test\Pastedimage20231109195759.png)

**欲找到具有“最大间隔”(maximum margin)的划分超平面，也就是要找到能满足式(6.3)中约束的参数w和b，使得r最大。这就是支持向量机（SVM）的基本型。**
![](test\Pastedimage20231109200116.png)

![](test\Pastedimage20231110113601.png)
...
![](test\Pastedimage20231110122706.png)
...
VC Bound 的一般形式为：
![](test\Pastedimage20231128133041.png)
其中：
